...
neo4j_driver = GraphDatabase.driver(
  "bolt://3.234.154.25:7687",
  auth=basic_auth("neo4j", "saddle-clouds-catchers"))
...
for i in range(len(query_results)):
  for j in range(len(query_results[i])):
    if query_results[i][j] is not None:
      string = query_results[i][j] + delimiter + str(userdict_number) + delimiter + userdict_label[i]
